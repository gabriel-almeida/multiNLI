...
class MyModel(object):
    def __init__(self, seq_length, emb_dim, hidden_dim, embeddings, emb_train):
        ...
        def network(premise, hypothesis, reuse=False):
            ...
            prem_seq_lengths, mask_prem = blocks.length(premise)
            hyp_seq_lengths, mask_hyp = blocks.length(hypothesis)
            ...
            premise_outs, c1 = blocks.biLSTM(premise_in, dim=self.dim, seq_len=prem_seq_lengths, name='premise', reuse=reuse)
            hypothesis_outs, c2 = blocks.biLSTM(hypothesis_in, dim=self.dim, seq_len=hyp_seq_lengths, name='hypothesis', reuse=reuse)
            ...
            alpha_s = tf.stack(alphas, axis=2)
            beta_s = tf.stack(betas, axis=2) 
            ...
            v1_outs, c3 = blocks.biLSTM(m_a, dim=self.dim, seq_len=prem_seq_lengths, name='v1', reuse=reuse)
            v2_outs, c4 = blocks.biLSTM(m_b, dim=self.dim, seq_len=hyp_seq_lengths, name='v2', reuse=reuse)
            ...
            return tf.matmul(h_drop, self.W_cl) + self.b_cl, alpha_s, beta_s

        # Get prediction
        self.logits, self.alpha_s, self.beta_s = network(self.premise_x, self.hypothesis_x)
        self.reversed_logits, _, _ = network(self.hypothesis_x, self.premise_x, reuse=True)
